...
def send_command(linear: float, angular: float) -> tuple[bool, str]:
    ...
    # Try browser-relay first, fall back to direct RTM
    try:
        response = requests.post(CONTROL_URL, json=payload, timeout=1)
        if response.ok:
            return True, "ok"
    except Exception:
        pass
    try:
        response = requests.post(CONTROL_LEGACY_URL, json=payload, timeout=2)
        if response.ok:
